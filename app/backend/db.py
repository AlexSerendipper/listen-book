...
from .mobile.migrations import run_mobile_migrations
...
def init_db(db_path: Path = DB_PATH, *, include_mobile: bool = True) -> None:
    ensure_dirs()
    with _INIT_LOCK:
        ...
        try:
            ...
            _ensure_column(
                conn, "reading_progress", "has_playback_position", "INTEGER NOT NULL DEFAULT 1"
            )
            ...
            _ensure_column(
                conn, "reading_progress", "reading_page_offset", "INTEGER NOT NULL DEFAULT 0"
            )
            conn.commit()
            if include_mobile:
                run_mobile_migrations(conn)
        finally:
            conn.close()
