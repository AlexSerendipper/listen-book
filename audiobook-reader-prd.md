...
- 窗口尺寸变化触发异步分页重算时，必须保留当前播放句和视觉页锚点；分页重算开始后的用户点击应优先于重算前捕获的旧页面状态。
- 书库必须作为脱离正文布局的悬浮抽屉，默认关闭；打开或关闭不得改变阅读区尺寸、触发分页重算、影响播放或移动当前视觉页。
- “回到当前页”按钮应以最后播放句所在视觉页为目标。当前浏览页包含该句时禁用，否则启用；同一段落内浏览到其他视觉页也不能错误置灰。
...
-> 生成或识别 book_id
-> 查询 reading_progress
-> 如果存在阅读锚点，先按当前布局恢复到对应内容所在视觉页
-> 如果存在播放位置，保留 chapter_index + paragraph_index + audio_position_ms 作为播放目标
-> “回到当前页”可从阅读页返回最后播放句所在视觉页
-> 如果不存在历史进度，从第一章第一段开始
```
...
- 书库默认为关闭状态；书名左侧的书本图标打开悬浮抽屉，点击抽屉外区域或按 `Esc` 关闭。
- 书库抽屉必须脱离正文布局并覆盖在文字上方，宽屏沿用侧栏宽度、窄屏可接近全屏，但任何状态都不能挤压阅读面板。
...
- 可以保存每本书的听书进度
- 可以独立保存最后阅读内容锚点和最后播放位置
- 关闭应用后重新打开同一本书，初始显示最后阅读内容；“回到当前页”可返回最后播放句
- 打开或关闭书库不改变阅读区宽高、分页结果、当前视觉页或播放状态
...
问题场景：播放当前视觉页中的句子，或点击该页中的某句话开始播放；音频自然进入下一句时，阅读区偶发回到上一视觉页。旧版书库展开/收起会改变阅读区尺寸并提高出现概率，但不是书籍内容或导入缓存导致的问题。
...
回归场景：在接近问题页面总数的浏览器尺寸下调整窗口触发分页重算，不等待重算完成，真实点击目标句的上一句，从该句开头以 1 倍速自然播放到目标句；断言活动句正确前进，视觉页索引、实际列索引和横向滚动位置均不向前移动。该修复不要求重新导入书籍。

### 12.2 已解决问题：书库重分页与“回到当前页”误置灰

记录日期：2026-07-26。

问题场景：同一朗读段跨多个视觉页时，用户翻到不包含当前播放句的页面，“回到当前页”仍可能置灰；打开或关闭书库还会改变正文宽度并触发整本书分页重算。

根因：“回到当前页”的状态只在部分渲染路径更新，视觉翻页后可能保留旧状态；旧书库属于主网格列，展开/收起必然改变阅读区宽度。

修复约束：

- 每次视觉页定位后重新比较当前浏览页与最后播放句所在页。
- 返回播放页重新渲染章节时，只要渲染的是播放章节就必须恢复句子定时映射和活动高亮，不能以重建瞬间的可见性作为恢复条件。
- 暂停或停止后继续保留最后播放位置；尚未播放时使用阅读锚点恢复初始页。
- 最后阅读位置保存内容锚点而非固定视觉页码，窗口尺寸变化后仍定位到同一段内容。
- 书库改为默认关闭的固定定位悬浮抽屉，开关不调用分页重算；遮罩点击和 `Esc` 可关闭，播放与高亮不受影响。

验收场景：播放并暂停一句后翻到同段其他视觉页，按钮必须启用并准确返回该句；继续翻阅后关闭并重新打开书籍，应先恢复最后阅读内容，按钮仍能返回播放页。反复打开和关闭书库时，分页尺寸键、视觉页索引、正文横向位置、音频状态和高亮句必须保持不变。
