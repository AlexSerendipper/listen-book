...
const chapterMenuScreenshot = "C:/Users/alexzhong/.codex/visualizations/2026/08/02/019fc125-e748-7602-af9b-ad325e72f3db/mobile-pwa-chapter-menu-v9.png";
const errors = [];
...
assert.equal(await offlinePage.locator("#pageIndicator").innerText(), secondPageLabel);
assert.equal(await offlinePage.locator("#readerMenuButton").count(), 0);
await tapReader(offlinePage, 195);
await offlinePage.locator("#readerMenuLayer").waitFor({ state: "visible" });
assert(await offlinePage.locator('[data-menu-panel="root"]').isVisible());
await swipeLeft(offlinePage);
assert.equal(await offlinePage.locator("#pageIndicator").innerText(), secondPageLabel, "menu should lock swipe navigation");
await offlinePage.getByRole("button", { name: "目录", exact: true }).click();
assert(await offlinePage.locator('[data-menu-panel="chapters"]').isVisible());
assert.equal(await offlinePage.locator('.reader-chapter-item[aria-current="location"] strong').innerText(), "第一章");
await offlinePage.screenshot({ path: chapterMenuScreenshot });
await offlinePage.locator("#readerMenuBackdrop").click({ position: { x: 20, y: 80 } });
assert(await offlinePage.locator('[data-menu-panel="root"]').isVisible());
await offlinePage.locator("#readerMenuBackdrop").click({ position: { x: 20, y: 80 } });
assert.equal(await offlinePage.locator("#readerMenuLayer").isVisible(), false);
await tapReader(offlinePage, 195);
await offlinePage.getByRole("button", { name: "字体", exact: true }).click();
assert(await offlinePage.locator('[data-menu-panel="font"]').isVisible());
await offlinePage.locator("#readerMenuBackdrop").click({ position: { x: 20, y: 80 } });
assert(await offlinePage.locator('[data-menu-panel="root"]').isVisible());
await offlinePage.locator("#readerMenuBackdrop").click({ position: { x: 20, y: 80 } });
await tapReader(offlinePage, 195);
await offlinePage.getByRole("button", { name: "目录", exact: true }).click();
await offlinePage.locator('.reader-chapter-item[data-chapter-index="0"]').click();
await offlinePage.waitForFunction(() => document.querySelector("#pageIndicator").textContent === "第 1/2 章 · 1/1 页");
assert.equal(await offlinePage.locator("#readerMenuLayer").isVisible(), false);
await tapReader(offlinePage, 195);
await offlinePage.getByRole("button", { name: "目录", exact: true }).click();
assert.equal(await offlinePage.locator('.reader-chapter-item[aria-current="location"] strong').innerText(), "扉页");
await offlinePage.locator('.reader-chapter-item[data-chapter-index="1"]').click();
await offlinePage.waitForFunction(() => /^第 2\/2 章 · 1\/\d+ 页$/.test(document.querySelector("#pageIndicator").textContent));
await tapReader(offlinePage, 330);
await offlinePage.waitForFunction(() => /^第 2\/2 章 · 2\/\d+ 页$/.test(document.querySelector("#pageIndicator").textContent));
const savedBook = await offlinePage.evaluate(async (hashValue) => {
  const database = await import("/mobile/src/db.js?v=9");
